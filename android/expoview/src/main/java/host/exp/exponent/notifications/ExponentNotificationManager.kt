...
class ExponentNotificationManager(private val context: Context) {
  ...
  fun readChannelSettings(experienceKey: ExperienceKey?, channelId: String?): JSONObject? {
    try {
      var metadata = exponentSharedPreferences.getExperienceMetadata(
        experienceKey!!
      )
      if (metadata == null) {
        metadata = JSONObject()
      }
      val allChannels: JSONObject
      allChannels =
  ...
  fun notify(experienceKey: ExperienceKey, id: Int, notification: Notification?) {
    ...
    try {
      ...
      var notifications =
        metadata.optJSONArray(ExponentSharedPreferences.EXPERIENCE_METADATA_ALL_NOTIFICATION_IDS)
      if (notifications == null) {
        notifications = JSONArray()
      }
      notifications.put(id)
      metadata.put(
        ExponentSharedPreferences.EXPERIENCE_METADATA_ALL_NOTIFICATION_IDS,
        notifications
      )
      exponentSharedPreferences.updateExperienceMetadata(experienceKey, metadata)
    } catch (e: JSONException) {
  ...
  fun getAllNotificationsIds(experienceKey: ExperienceKey?): List<Int> {
    return try {
      ...
      val notifications =
        metadata.optJSONArray(ExponentSharedPreferences.EXPERIENCE_METADATA_ALL_NOTIFICATION_IDS)
          ?: return emptyList()
      val notificationsIds: MutableList<Int> = ArrayList()
      for (i in 0 until notifications.length()) {
  ...
  fun schedule(
    ...
    notificationIntent.type = experienceKey.scopeKey
    ...
    try {
      ...
      var notifications =
        metadata.optJSONArray(ExponentSharedPreferences.EXPERIENCE_METADATA_ALL_SCHEDULED_NOTIFICATION_IDS)
      if (notifications == null) {
        notifications = JSONArray()
      }
      notifications.put(id)
      metadata.put(
        ExponentSharedPreferences.EXPERIENCE_METADATA_ALL_SCHEDULED_NOTIFICATION_IDS,
        notifications
      )
      exponentSharedPreferences.updateExperienceMetadata(experienceKey, metadata)
    } catch (e: JSONException) {
  ...
  companion object {
    ...
    private val notificationChannelGroupIds: MutableSet<String> = HashSet()
