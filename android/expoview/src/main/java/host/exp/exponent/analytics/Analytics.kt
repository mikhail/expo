...
object Analytics {
  ...
  const val ERROR_APPEARED = "ERROR_APPEARED"
  const val EXPERIENCE_APPEARED = "EXPERIENCE_APPEARED"
  const val LOAD_EXPERIENCE = "LOAD_EXPERIENCE"
  const val RELOAD_EXPERIENCE = "RELOAD_EXPERIENCE"
  const val ERROR_SCREEN = "ERROR_SCREEN"
  const val ERROR_RELOADED = "ERROR_RELOADED"
  const val HTTP_USED_CACHE_RESPONSE = "HTTP_USED_CACHE_RESPONSE"
  const val HTTP_USED_EMBEDDED_RESPONSE = "HTTP_USED_EMBEDDED_RESPONSE"
  const val LOAD_DEVELOPER_MANIFEST = "LOAD_DEVELOPER_MANIFEST"
  ...
  fun logEvent(eventType: String, eventProperties: JSONObject) {
    if (!Constants.ANALYTICS_ENABLED) {
      return
    }
  ...
  fun logEventWithManifestUrl(eventType: String, manifestUrl: String?) {
  ...
  fun logEventWithManifestUrlSdkVersion(
    ...
    if (!Constants.isStandaloneApp() && eventType != LOAD_EXPERIENCE) {
      return
    }
  ...
  fun sendTimedEvents(manifestUrl: String?) {
    ...
    try {
      ...
      logEvent(if (isShell) "SHELL_EXPERIENCE_LOADED" else "EXPERIENCE_LOADED", eventProperties)
    } catch (e: Exception) {
      EXL.e(TAG, e.message)
    } finally {
      shellTimedEvents.clear()
