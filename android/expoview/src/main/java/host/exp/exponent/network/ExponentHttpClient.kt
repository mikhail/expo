...
class ExponentHttpClient(
  ...
  fun call(request: Request, callback: ExpoHttpCallback) {
    okHttpClientFactory.newClient.newCall(request).enqueue(object : Callback {
  ...
  fun tryForcedCachedResponse(
    ...
    okHttpClientFactory.newClient.newCall(newRequest).enqueue(object : Callback {
      override fun onFailure(call: Call, e: IOException) {
